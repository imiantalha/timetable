...
export type GenerationAssignment={course_id:number;teacher_id:number;section_id:number;room_id:number};
...
export const getDepartments=()=>getOptions('departments');export const getSections=(departmentId?:number)=>getOptions('sections',{department_id:departmentId});export const getCourses=(departmentId?:number)=>getOptions('courses',{department_id:departmentId});export const getTeachers=(departmentId?:number)=>getOptions('teachers',{department_id:departmentId});export const getRooms=()=>getOptions('rooms');export const getTimeSlots=()=>getOptions('time-slots');
...
export async function generateTimetable(timetableId:number,assignments:GenerationAssignment[],timeSlotIds:number[]){const{data}=await api.post(`/timetables/${timetableId}/generate`,{assignments,time_slot_ids:timeSlotIds});return data.data;}
